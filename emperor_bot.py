        ...
        if message.text:
            ...
            if '@HoraryEmperorBot' in message.text:
                ...
                if not bot_text or len(bot_text) < 3:
                    choice_text = """
👑 *Профессиональный Хорарный Анализ*

Я готов провести глубинный астрологический разбор вашей ситуации.

💬 *Варианты обращения:*
• Задайте вопрос здесь — публичный анализ
• Напишите "Личное [вопрос]" — ответ в ЛС  
• Напишите мне в личные сообщения — полная конфиденциальность

*Примеры профессиональных вопросов:*
• «Придут ли мне деньги до пятницы?»
• «Любит ли меня этот человек?»
• «Как улучшить мое здоровье?»
"""
                ...
                if bot_text.lower().startswith('личное'):
                    ...
                    if question:
                        try:
                            ...
                            bot.reply_to(message, "📨 *Отправил профессиональный анализ в ваши личные сообщения!*")
                        except:
                            bot.reply_to(message, "❌ *Напишите мне сначала в личные сообщения!*")
                    return
                else:
                    question = bot_text
...
Готов провести глубинный астрологический анализ вашей ситуации на основе хорарных карт.
...
*Пример:* «Придут ли мне деньги за проект до конца месяца?»
"""
                ...
                if question:
                    try:
                        ...
                        bot.send_message(message.from_user.id, private_msg)
                        bot.reply_to(message, "📨 *Отправил анализ в личные сообщения!*")
                    except:
                        bot.reply_to(message, "💌 *Напишите мне сначала в личные сообщения!*")
                return
...
*Глубинный астрологический анализ на основе хорарных карт*
...
• ✈️ *Поездки* — стоит ли ехать, будет ли удачное путешествие
...
"""
    ...
    try:
        if len(message.text.strip()) < 5 or message.text.lower().strip() in ['император', 'бот', 'привет']:
            responses = [
                "👑 *Задайте вопрос для профессионального астрологического анализа!*",
                "🔮 *Что вас волнует? Финансы, отношения, здоровье?*",
                "💫 *Готов к глубинной астрологической диагностике вашей ситуации!*"
            ]
        ...
        response = f"""
{verdict} *ПРОФЕССИОНАЛЬНЫЙ ХОРАРНЫЙ АНАЛИЗ*
⏰ {display_time}, МОСКВА
...
# 🔧 ИСПРАВЛЕНИЕ ОШИБКИ 409 - добавляем паузу и перезапуск
try:
    print("🔗 Подключаемся к Telegram...")
    time.sleep(5)  # Пауза перед подключением
    bot.remove_webhook()
